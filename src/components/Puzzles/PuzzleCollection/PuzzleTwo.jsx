...
const Cube = styled.div`
  ...
  &:active {
    transform: ${(props) => !props.$disabled && "scale(0.92)"};
  }
`;
...
const PuzzleTwo = ({ onExit }) => {
  ...
  const handleCubeClick = (index) => {
    if (isShowing || isWon || isError || isHinting) return;
    ...
    >
      <div style={{ height: "40px", zIndex: 60 }}>
        {isWon ? (
          ...
          <h2 style={{ color: "#ff5252", margin: 0 }}>Помилка!</h2>
        ) : (
          <h3
            style={{ color: pendingRestart ? "#4caf50" : "#ffb36c", margin: 0 }}
          >
            {pendingRestart
              ? "Буде перезапущено..."
              : isShowing
      ...
      <Grid $grid={config.grid} className={isError ? "shake" : ""}>
        {[...Array(config.grid * config.grid)].map((_, i) => (
          <Cube
            key={i}
            $active={activeCube === i}
            $disabled={isShowing || isError || isHinting}
            $speed={config.speed}
      ...
      <BottomPanel>
        ...
        <Controls>
          ...
          <GameButton
            className={
              pendingRestart && !pendingRestart.config ? "pending" : ""
            }
            onClick={() =>
              isShowing ? setPendingRestart({ config: null }) : startNewLevel()
            }
            disabled={isError}
          >
            {pendingRestart && !pendingRestart.config ? "⏳" : "⏭"}
          </GameButton>
          <GameButton onClick={onExit} disabled={isShowing || isError}>
            ✖
          </GameButton>
        </Controls>
      </BottomPanel>
            ...
            <Modal onClick={(e) => e.stopPropagation()}>
              ...
              >
                <DifficultyBtn onClick={() => applyDifficulty("easy")}>
                  Легка: 2x2, 5 кроків, 1c/кл.
                </DifficultyBtn>
                <DifficultyBtn onClick={() => applyDifficulty("normal")}>
                  Середня: 3x3, 7 кроків, 0.7c/кл.
                </DifficultyBtn>
                <DifficultyBtn onClick={() => applyDifficulty("hard")}>
                  Екстремальна: 4x4, 10 кроків, 0.5c/кл.
                </DifficultyBtn>
              </div>
              ...
              <CustomRow>
                <span>
                  Сітка: {tempConfig.grid}x{tempConfig.grid}
                </span>
                <input
                  type="range"
                  min="2"
                  max="4"
                  value={tempConfig.grid}
                  onChange={(e) =>
                    setTempConfig({
                      ...tempConfig,
